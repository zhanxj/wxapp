...
class coupon_app{
     function getCoupon(){
         global $_W, $_GPC;
         $couponid = $_GPC['id'];
         $coupons = Coupon :: getSingleCoupons($couponid, '*');
         $num = Coupon :: getCouponNum($couponid, 1);
         if($coupons['time_type'] == 1 && $coupons['endtime'] < time()){
            wl_message('抱歉，优惠券已过期', app_url('coupon/coupon_app/couponList'), 'error');
        }
         if($coupons['status'] == 0){
            wl_message('抱歉，优惠券已被禁用', app_url('coupon/coupon_app/couponList'), 'error');
        }
         if($coupons['status'] == 3){
            wl_message('抱歉，优惠券已经失效', app_url('coupon/coupon_app/couponList'), 'error');
        }
         if($coupons['surplus'] > ($coupons['quantity'] - 1)){
            wl_message('抱歉，优惠券已经被领光了', app_url('coupon/coupon_app/couponList'), 'error');
        }
         if($num){
             if(($num > $coupons['get_limit'] || $num == $coupons['get_limit']) && $coupons['get_limit'] > 0){
                 wl_message('抱歉，一个用户只能领取' . $coupons['get_limit'] . '张。', app_url('coupon/coupon_app/couponList'), 'error');
                 }
             }
         $newsurplus = $coupons['surplus'] + 1;
         Coupon :: updateCoupons(array('surplus' => $newsurplus), array('id' => $couponid));
         ...
         $res = Coupon :: saveMemberCoupons($data);
         if($res){
             wl_message('恭喜你获得一张优惠券', app_url('coupon/coupon_app/couponList'), 'success');
             }else{
             wl_message('获取优惠券失败', app_url('coupon/coupon_app/couponList'), 'error');
             }
         }
     ...
     function createCouponorder(){
         ...
         $num = Coupon :: getCouponNum($couponid, 2);
         $couponnum = $_GPC['num'];
         $allnum = $num + $couponnum;
         if($coupons['time_type'] == 1 && $coupons['endtime'] < time()){
            wl_message(array('errno' => 1, 'message' => '抱歉，优惠券已过期'));
        }
         ...
         if($allnum > $coupons['get_limit'] && $coupons['get_limit'] > 0){
             wl_message(array('errno' => 1, 'message' => '抱歉，一个用户只能购买' . $coupons['get_limit'] . '张。'));
             }
         $data = array(
            'uniacid' => $_W['uniacid'],
             'mid' => $_W['mid'],
         ...
         $res = Coupon :: saveCouponOrder($data);
         if($res){
     ...
     function cash(){
         global $_W, $_GPC;
         $order = pdo_get('wlmerchant_order', array('id' => $_GPC['orderid']));
         $coupon = Coupon :: getSingleCoupons($order['fkid'], '*');
         $order['title'] = $coupon['title'];
         wlPay :: getCash($_GPC['paytype'], $order['orderno'], $order['title'], $order['price'], 'Couponsharge', 'Coupon', $_GPC['done']);
         }
     ...
     function getcoupons(){
         ...
         $couponlist = Coupon :: getNumCoupon('*', $where, 'ID DESC', 0, 0, 0);
     ...
     function getcoupondetail(){
         ...
         $parent = Coupon :: getSingleCoupons($res['parentid'], 'merchantid,promotion_url_name,promotion_url,promotion_title,logo');
         ...
         $res['promotion_url_name'] = $parent['promotion_url_name'];
         $res['promotion_url'] = $parent['promotion_url'];
         $res['promotion_title'] = $parent['promotion_title'];
         $url = app_url('coupon/coupon_app/hexiaocoupon', array('id' => $id));
         $res['qrimgurl'] = app_url('coupon/coupon_app/qrcodeimg', array('url' => $url));
     ...
     public function getstore(){
        ...
        $locations = Coupon :: getNumstore('*', $parm, 'ID DESC', 0, 0, 0);
         $locations = $locations[0];
         $locations = Coupon :: getstores($locations, $lng, $lat);
        ...
        foreach ($locations as $key => & $v){
             ...
             if($coupon){
                 foreach ($coupon as $k => & $asd){
                     ...
                     $asd['href'] = app_url('coupon/coupon_app/couponsdetail', array('id' => $asd['id']));
                     }
                 $v['coupon'] = $coupon;
                 }else{
                 $v = '';
     ...
     public function couponsdetail(){
         ...
         $pagetitle = !empty($_W['wlsetting']['base']['name']) ? '卡券详情 - ' . $_W['wlsetting']['base']['name'] : '卡券详情';
         $id = $_GPC['id'];
         $coupons = Coupon :: getSingleCoupons($id, '*');
        ...
        $coupons['description'] = unserialize($coupons['description']);
         ...
         $url = app_url('coupon/coupon_app/tips', array('id' => $id));
     ...
     function tips(){
         global $_W, $_GPC;
         $id = $_GPC['id'];
         wl_message('请点选下方按钮领取或购买', app_url('coupon/coupon_app/couponsdetail', array('id' => $id)), 'error');
         }
     ...
     function topayCoupon(){
         ...
         $url = app_url('coupon/coupon_app/topayCoupon', array('id' => $id));
     ...
     function hexiaocoupon(){
         ...
         $coupon = Coupon :: getSingleCoupon($id, 'status,usedtime,endtime,usetimes,parentid');
         if($password){
             if($password != 'asdasd'){
                 ...
                 $res = Coupon :: updateCoupon(array('usetimes' => $usetimes, 'usedtime' => $coupon['usedtime']), array('id' => $id));
                 if($res){
             ...
             $parent =
             $verifier = SingleMerchant :: verifier($coupon['merchantid'], $_W['mid']);
             if($verifier){
                 $coupon = Coupon :: getSingleCoupon($id, 'status,usedtime,endtime,usetimes');
                 if($coupon['usetimes'] == 0){
                     wl_message('该优惠券已失效', 'close', 'error');
                     }
                 if($coupon['endtime'] < time()){
