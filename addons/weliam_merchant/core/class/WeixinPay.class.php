<?php class WeixinPay{
    ...
    public function finance($openid = '', $money = 0, $desc = ''){
        global $_W;
        $setting = uni_setting($_W['uniacid'], array('payment'));
        $data = array();
        ...
        $data['nonce_str'] = $this -> createNoncestr();;
        $data['partner_trade_no'] = time() . random(4, true);
        $data['openid'] = $openid;
    ...
    public function wxHttpsRequestPem($vars, $url, $second = 30, $aHeader = array()){
        ...
        curl_setopt($ch, CURLOPT_SSLCERT, PATH_DATA . "cert/" . $_W['uniacid'] . "/wechat/apiclient_cert.pem");
