...
class Pay_EweiShopV2Page extends AppMobilePage
{
	...
	protected function success($orderid)
	{
		...
		app_json(array('order' => array('id' => $orderid, 'isverify' => $order['isverify'], 'virtual' => $order['virtual'], 'isvirtual' => $order['isvirtual'], 'isvirtualsend' => $order['isvirtualsend'], 'status' => $order['paytype'] == 3 ? '订单提交支付' : '订单支付成功', 'text' => $text, 'price' => $order['price']), 'paytype' => $order['paytype'] == 3 ? '需到付' : '实付金额', 'carrier' => $carrier, 'address' => $address, 'stores' => $stores, 'store' => $store, 'icon' => $icon));
	}
